...
export const updateTotalPrice = () => {
  ...
  const cartProducts = document.querySelector('#cart-list');
  const prices = cartProducts.querySelectorAll('.product__price__value');
  const operationPrice = [...prices]
    .map((price) => {
      const formattedPrice = parseFloat(price.innerHTML).toFixed(2);

      return parseFloat(formattedPrice);
    })
    .reduce((accumulator, price) => {
      accumulator += price;
      return parseFloat(accumulator.toFixed(2));
    }, 0);

  totalPrice.innerHTML = operationPrice;
};
...
export const createCartProductElement = ({ id, title, price, pictures }) => {
  ...
  const priceCC = createCustomElement('div', 'container-value', '');
  const priceElement = createCustomElement('p', 'product__price', 'R$ ');
  priceElement.appendChild(createCustomElement('span', 'product__price__value', price));
  priceCC.appendChild(priceElement);
  infoContainer.appendChild(priceCC);
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  const priceCC = createCustomElement('div', 'container-value', '');
  const priceElement = createCustomElement('p', 'product-value', 'R$ ');
  priceElement.appendChild(createCustomElement('span', 'product__price__value', price));
  priceCC.appendChild(priceElement);
  ...
  priceCC.appendChild(cartButton);
  section.appendChild(priceCC);
